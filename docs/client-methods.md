...
С сервисом Employee можно взаимодействовать через клиент. В этом файле — методы, они позволяют создавать, получать, изменять и выполнять другие действия с сущностями: сотрудниками `Person`, должностями `Position` и ролями `Role`. Подробнее о сущностях можно прочитать в [_entities.md_](/docs/entities.md).

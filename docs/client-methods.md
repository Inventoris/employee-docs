...
С сервисом Employee можно взаимодействовать через клиент. В этом файле описаны методы клиента, которые позволяют создавать, получать, изменять и удалять сущности: сотрудников `Person`, должности `Position` и роли `Role`. Подробнее о сущностях можно прочитать в [_entities.md_](/docs/entities.md).
